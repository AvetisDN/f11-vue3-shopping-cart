  ...
  <Card class="p-my-1">
    ...
    <template #footer>
      <div class="p-d-flex p-flex-row p-justify-between p-align-center">
        ...
        <Button :icon="inCart ? 'pi pi-check-circle' : 'pi pi-plus-circle'" :label="inCart ? 'In Cart' : 'Buy'" @click="addToCart" :disabled="inCart" />
      </div>
    </template>
  </Card>
</template>
...
export default {
  ...
  setup(props) {
    ...
    inCart.value = store.getters.isInCart(props.product.id)
